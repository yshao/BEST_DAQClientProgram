...
### ui files
from ui_visbar import Ui_PlotDialog
from best.commonutils import *
from best.commontestutils import *
from best.commonsqliteutils import *
from best.commonconfigutils import *
from best.commonfileutils import *
...
class PlotThread(QtCore.QThread):
    data_plotted = QtCore.pyqtSignal(object)

    def __init__(self, file):
        QtCore.QThread.__init__(self)
        self.file=file
        self.queue=Queue()
        self.writer = Process(target=plotdata, args=(self.queue,))
    ...
    def run(self):
        ...
        self.writer.start()
        self.writer.join()
        self.data_plotted.emit('%s' % (self.file))

    def update(self,queue):
      while 1:
        msg = queue.get()         # Read from the queue and do nothing
        if (msg == 'result: '):
            self.data_plotted.emit('%s' % self.parse_result(msg))
            break
        else:
            self.data_plotted.emit('%s' % (msg))

    def parse_result(self,msg):
        ""
        # return "PARSED: "+msg
        self.data_plotted.emit('Result: %s' % (msg))
