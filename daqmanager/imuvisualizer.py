...
# First scene (Roll, Pitch, Yaw)
scene3 = display(title='XSens IMU test',x=0, y=0, width=500, height=200,center=(0,0,0), background=(0,0,0))
scene3.range=(1,1,1)
...
scene3.select()
#Roll, Pitch, Yaw
cil_roll3 = cylinder(pos=(-0.4,0,0),axis=(0.2,0,0),radius=0.01,color=color.red)
cil_roll4 = cylinder(pos=(-0.4,0,0),axis=(-0.2,0,0),radius=0.01,color=color.red)
cil_pitch3 = cylinder(pos=(0.1,0,0),axis=(0.2,0,0),radius=0.01,color=color.green)
cil_pitch4 = cylinder(pos=(0.1,0,0),axis=(-0.2,0,0),radius=0.01,color=color.green)
#cil_course = cylinder(pos=(0.6,0,0),axis=(0.2,0,0),radius=0.01,color=color.blue)
#cil_course2 = cylinder(pos=(0.6,0,0),axis=(-0.2,0,0),radius=0.01,color=color.blue)
arrow_course2 = arrow(pos=(0.6,0,0),color=color.cyan,axis=(-0.2,0,0), shaftwidth=0.02, fixedwidth=1)
...
L4 = label(pos=(-0.4,0.22,0),text="-",box=0,opacity=0)
L5 = label(pos=(0.1,0.22,0),text="-",box=0,opacity=0)
L6 = label(pos=(0.7,0.3,0),text="-",box=0,opacity=0)
...
#f = open("SerialRazor"+str(time())+".txt", 'w')
#f2 = open("SerialXSens"+str(time())+".txt", 'w')
...
#    # print line
    data=ser2.read()
    line = xsens_parse(data)
    f2.write(line)                     # Write to the output log file
    ###
    # data is roll,pitch,yaw def xsens_parse(data)
    #
...
#    words = string.split(line,",")    # Fields split
    ...
    if len(words) > 2:
        ...
        cil_pitch2.axis=(-0.2*cos(pitch),-0.2*sin(pitch),0)
        arrow_course.axis=(0.2*sin(yaw),0.2*cos(yaw),0)
        L4.text = str(float(words[0]))
        L5.text = str(float(words[1]))
        L6.text = str(float(words[2]))        
